...
PARAM  <- list()
PARAM$experimento  <- "CLU7610"
# FIN Parametros del script

#------------------------------------------------------------------------------
#------------------------------------------------------------------------------

# ATENCION :  cambiar por SU carpeta
setwd( "~/buckets/b1/" ) 

#leo el dataset original
# pero podria leer cualquiera que tenga Feature Engineering
dataset  <- fread( "./datasets/competencia_2023_.csv.gz", stringsAsFactors= TRUE)
...
# por supuesto, se pueden cambiar .. se lo invita a que los cambie
...
tb_final  <- dataset[, lapply(.SD, mean), by=cluster2 ]
 
fwrite( tb_final,
        file= "clusters_centroides.txt",
        sep= "\t" )


#ahora a mano veo los centroides de los 7 clusters
dataset[ , list( "ctrx_quarter" = mean(ctrx_quarter)),  cluster2 ]  #media de la variable  ctrx_quarter
